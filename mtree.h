...
mnode *newNode(char *msg){
    ...
    strcpy(t->data, msg);
    t->count = 0;
    for(int i = 0; i < MAX; i++) t->child[i] = NULL;
    return t;
}
...
mnode* loadTree(){
    ...
    while(fgets(line, 500, fp)){
        ...
        if(start) insertPath(root, start + 1);
    }
    fclose(fp);
    return root;
}
...
mnode *search(mnode *root, char *key){
    if(!root) return NULL;
    if(compareIgnoreCase(root->data, key)) return root;
...
mnode *findNode(mnode *root, char *name){
    if(!root) return NULL;
    if(compareIgnoreCase(root->data, name)) return root;
    for(int i = 0; i < root->count; i++){
...
int deleteNodeHelper(mnode *root, char *key){
    ...
    for(int i = 0; i < root->count; i++){
        ...
        if(deleteNodeHelper(root->child[i], key)) return 1;
    }
    return 0;
}
...
void addNode(mnode *root, char *parentName, char *childName){
    ...
    if(!parent){ printf("Parent '%s' not found!\n", parentName); return; }
    ...
    if(parent->count >= MAX){ printf("Max children reached!\n"); return; }
...
int printPath(mnode *root, char *target, char path[][50], int depth){
    ...
    for(int i = 0; i < root->count; i++)
        if(printPath(root->child[i], target, path, depth + 1)) return 1;
    return 0;
}
...
void navigate(mnode *start, mnode *root, int loggedIn, trie *users){
    ...
    while(1){
        ...
        //Back 
            ...
            if(prev) curr = prev;
            else      printf("Already at top!\n");
        }

        //Undo 
        else if(ch == -1){
            deletedInfo *info = (deletedInfo *)pop(&undoStack);
            if(!info){ printf("Nothing to undo!\n"); }
            else{
        ...
        //Add (permanent) 
            ...
            printf("Enter parent node: ");
            getchar();
            fgets(parent, 100, stdin); parent[strcspn(parent, "\n")] = 0;
            printf("Enter new child: ");
            fgets(child,  100, stdin); child[strcspn(child,  "\n")] = 0;
            addNode(root, parent, child);
            saveTree(root);
        }
